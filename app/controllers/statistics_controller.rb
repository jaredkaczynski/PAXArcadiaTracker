class StatisticsController < ApplicationController
  ...
  def charts
    ...
    first_time = -1
    #Set first time to 1553781600 for live
    data_builder_completion_speed = ""
    data_builder_total_completions = ""
    sorted.each_with_index  do |p, index|
      if first_time == -1
        first_time = p.time_completed.to_i
      end
      data_builder_total_completions << "{ x: " + ((p.time_completed.to_i-first_time)%604800/3600).to_s + ", y: " + index.to_s + "},"
      if p.time_taken < lowest_time
        lowest_time = p.time_taken
        data_builder_completion_speed << "{ x: " + ((p.time_completed.to_i-first_time)%604800/3600).to_s + ", y: " + (p.time_taken/60).to_s + "},"
      end
    end
